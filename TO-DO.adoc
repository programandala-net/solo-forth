...
+3DOS `(cat` returns ior #-1020 (bad filename) in some cases:

- When `cat`/`wcat` and `acat`/`wacat` are loaded (no matter the
  order), the first execution of any of them fails with ior #-1020
  before displaying anything.  Later executions of any of them work
  fine.
- When only `cat`/`wcat` is loaded, is works fine.
- When only `acat`/`wacat` is loaded, the first execution of any of
  them fails with ior #-1020 after displaying the first part of the
  catalogue (the first 10 files, hold in the buffer pointed by
  `cat-buffer`). Later executions work fine.

It seems the bug has been introduced after adding `acat` and `wacat`,
but the shared code has not changed, included `(cat`.
