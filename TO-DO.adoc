...
Use the "\e" prefix for extra memory variants?

Simplify `u.r`.

Remove the old `alias!` when the extra memory system becomes definitive.

Remove `split` and `join` from the library when the extra memory system
becomes definitive.
...
Make `noop` an alias, using `next` as xt. This saves `jp (ix)`.

Use `_jump` macros at the end of `umax`, `umin`, `dabs`, `abs`, etc.

Remove `join` and `split` from the library, moved to the kernel because of the
extra memory system.
...
names pointer temporarily. Update 2016-10-27: the extra memory is being implemented,
using 4 banks.
