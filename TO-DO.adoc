...
`set-drive`, `crnd`, `(delete-file)`, `n>r`, `nr>`, `d-`...
...
Rewrite `read-mode` and `write-mode` in Z80, to save some bytes.
...
Study a method to hide the different drive notations from the
application, in order to make the application work on any DOS.
...
And second, define a wrapper word to precede the operations:

----
: drive  ( x -- x' )  1+  ;             \ G+DOS
' noop alias drive  ( -- )  immediate   \ TR+DOS
: drive  ( x -- x' )  'A' +  ;          \ G+DOS
----

But the drive identifiers used in filenames will remain different.

All this can be solved by the application, if needed.
...
Make `2 set-drive throw` mandatory to start using library on the
second drive, like in TR-DOS. Now it's the default.

Anyway, the support for several library disks (planned for v0.14.0)
will make things simpler.

.2017-02-05:

Make `set-drive` return an ior, as the TR-DOS and +3DOS versions do.
...
[undefined] g+dos   ?\ 1 c, 2 c,
[undefined] tr-dos  ?\ 0 c, 1 c, 2 c, 3 c,
[undefined] +3dos   ?\ 'A' c, 'B' c,

: no-block-drive  ( -- )  block-drives max-drives erase  ;
...
: >drive-block  ( u1 -- u2 )
  ...
  block-drive@ ?dup ?noblock set-drive throw  ;
