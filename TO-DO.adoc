...
Replace "circular string buffer" with "`stringer`".

.2017-02-28:

Replace "This word" with the corresponding word name in
double backticks.
...
- Rename `block-drives` to `#block-drives`.
- Rename `(block-drives` to `block-drives`.
...
When the block number received by `list` is too big, the heading of
the listing is displayed anyway, and then:

- +3DOS throws #-1004 (no data), and any further attemp to list a real
  block throws #-1000 (drive not ready).
- G+DOS throws #-1004 (sector error).
- TR-DOS exits to BASIC with "Disc error Trk X sec X
  Retry,Abort,Ignore?".

.2017-03-05:

Improve `set-block-drives` and `block-drives`:

At the moment, `get-block-drives` depends on `set-block-drives`. The
problem is `(block-drives`, contrary to `block-drives`, is not in the
kernel. So, if `get-block-drives` was loaded before
`set-block-drives`, and `(block-drives` could be required without
`set-block-drives`, `block-drives` would contain 1 but `(block-drives`
would be empty... The simplest solution is to move `(block-drives` to
the kernel and combine it with `block-drive` into a single area.
...
Rewrite `cat` using the following information about the PCAT command
code, found in RAMSOFT's _DISCiPLE/+D Technical Guide_, Revision 8c
(30 Jan 2004), instead of direct calls to the ROM:
...
Rename `plusd-in` (and related) to `dos-in`, after the names used in
+3DOS.
...
Possible notation: "attra" for "screen attribute address",
and "scra" for "screen address".

Example:

- Rename `bitmap>attr-addr` to `scra>attra`.
- Rename `pixel-attr-addr` to `gxy>attra`.

Or maybe something with "display" instead of "screen".
