...
.2017-03-10, 2017-03-12, 2017-03-26, 2017-09-09:
...
----
: m3p ( 8b "name" -- )
  (c does> ( reg -- ) ( reg dfa ) %1111110 and c@ swap 8* + c, ;
...
Improve the documentation of constants and variables in the library
after the convention used in the kernel, i.e. "A [character]
constant|variable|value. _x_ is...".
...
Finish the documentation of <flow.case.fs>: `endcase`, `less-of`,
`greater-of`
...
Move the +3DOS first-boot initialization of banks 1346 to `cold`.
First, `cold` must be converted into a code word, and write `(cold)`.

.2017-12-07:

Move the default contents of `farlimit` and `far-banks`, which are
hardcoded in `cold`, to the parameter area. When these values are
changed by the application, probably the new values should be
preserved by `cold`. Therefore the application should be able to
configure also the defaults.
...
.2017-02-04, 2017-12-07:

NOTE: Milestone: 0.14.0

Make `cold` a code word in order to save space from basic init
operations, e.g. modifying memory. Then continue execution into an
unnamed colon word using `call do_colon`, to finish the high-level
tasks. [XXX UNDER DEVELOPMENT 2017-12-07].
