...
Rename "names bank" to "system bank", because it's used also for the history.
...
Fix? Now `number?` recognizes numbers that have no digit, only valid
punctuation, as zero.

.2016-03-16:

Idea for improving `number?` or an optional alternative: return the
chars and positions of every punctuation, not only the last one in
`dpl`.
