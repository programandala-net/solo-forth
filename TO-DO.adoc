...
- Make the relative control-flow structures optional and individually
  loadable.
- Simplify: Remove the `?pairs` checking, or make unchecked optional
  versions of the control-flow structures.

=== 2020-02-29

Replace `im1,` and `im2,` with `im,`; or add `im0` with the `m8`
definer.

Tests:

Replacing `im1,` and `im2,` with this code costs 17 bytes of code/data
...
This alternative, factoring `ed,` out of the four words that contain
`ED c,` (`sbcp,`, `adcp,`, `stp,` and `ftp,`), costs 12 bytes of
...
This variant costs 8 bytes of code/data space and saves 2 bytes of
name space:

----
: ed, ( -- ) ED c, ;
here 46 c, 56 c, 5E c,
: im, ( n -- ) ed, [ dup ] literal + c@ c, ; drop
----

=== 2020-02-29

- Make `addix` and `addiy` smart to detect `h` in order to avoid
  `ix-op c,` and `iy-op c,`, maxing the syntaxes `ix addix,` and `iy
  addiy,` possible. Currently a second $DD or $FD is compiled in those
  cases. Other solution: make `addp,` smart, as noted on 2017-03-12.
- Rewrite `exsp,` to accept `h`, `ix` or `ix`.
- Add `jpiy,`.
- Add `cpd,`, `cpdr,`, and `cpi,` with the `m8` definer.
- Add `ind,`, `indr,`, `ini,` and `inir,` with the `m8` definer.
- Add `otdr,` and `otir,` with the `m8` definer.
- Add `outd,` and `outi,` with the `m8` definer.
- Add `reti,` and `retn,` with the `m8` definer.

=== 2020-02-28

- Add `rrd,` with the `m8` definer.
- Make `stp,` and `ftp,` smarter, making `sthl,` and `fthl,`
  unnecessary.
...
`;code` is standard and cannot be reused, but anyway a simpler syntax
...
Benchmark `cells +` vs `swap array>`, used by `localized-string`
`far-localized-string` and `far>localized-string`.
...
Try `123. <# #S #> 456 . TYPE`. On popular systems such as Win32
Forth, VFX Forth and SwiftForth the output is 456 456. Gforth is a
notable exception as it outputs 456 123.
...
Factor Gforth's `set-does>` from `(;code)`.
...
Rename `name>string` to `name>stringer`.

Rename `name>str` to `name>string` (after Gforth)?

Rename `line>string` to `line>str`?
...
- Homogenize the pronunciation notation of acronyms in word names,
  e.g. after `dosior>ior` "dos-I-O-R-to-I-O-R", `file-ids` should be
  "file-I-D-s".
...
- Duplicate the Z80 instructions table with only two columns in order
  to make it fit in EPUB. Problem: The EPUB made from DocBook cannot
  be distinguised by an Asciidoctor condition; a specific version with
  an attribute would be needed.

=== 2020-07-12, 2020-07-27
...
- Add "Compile the Z80 jumps required" to assembler control-flow
  words.
- Cross-reference all Z80 conditions in assembler control-flow
  structures.

NOTE: Milestone: 0.15.0

- Search for words without cross-references in order to complete them.
- Search for words that have definitions whose main words are not
  cross-references.
- Cross-reference all words used in the definitions included in the
  glossary?
- Improve: "_a_ is the address of..." -> "Return address _a_ of...".
- Make cross-references to `{` and `}` (Hayest test) explicit?

=== 2020-05-03

- Document also the main words of the sample games.
- Finally, modify the Makefile to include all programs from the
  library (<prog.*.fs>) into the glossary.

=== 2020-02-29

- Fix rendering of footnotes in all formats (PDF, HTML and EPUB),
  especially in tables.
- Homogenize the transcription of names' "<" ("less-than", "from",
  "backwards").  and ">" ("greater-than", "to", "forwards").

=== 2020-02-27

Add this sections to the manual:

- A reference guide by subject, e.g. graphics, sound, blocks, files, control
  flow structures, etc.
...
Fix: The glossary cross references to Forth words that contain a backslash, or
that are included in code examples, are corrupted.
...
The HTML manual is >2 MiB.  Build it also in several linked parts.
...
Mark the credit notices and add them to an annex of the manual.

=== 2017-02-27

Create <doc/extra/> to hold external useful documents about Forth and
ZX Spectrum.
...
=== 2016-04-29, 2016-11-21, 2021-01-05

NOTE: Milestone: 0.15.0

Homogenize the stack notation for blocks and block lines.

Change _n_ to _u_ or _+n_ for blocks and block lines. _+n_ seems the
best option, because the maximum block number that can be stored in
`disk-buffer` is 32767-1.

Consult the
notation used in Forth-2012.
...
Fix `file-dir#` and `file-dirdesc`: they always return zero because of
a limitation in `(file-status`, which does not update the whole
`ufia`.
...
Fix `sectors-unused` (on which depend `drive-unused` and
`drive-used`): G+DOS only updates its internal variable $3DD8 when a
catalogue is displayed. Therefore `acat` is used. In order to solve
this problem, the `scan_cat` G+DOS routine should be copied and
...
file descriptor is hold. `read-fd` could be used instead, but "FD"
looks like "floppy disk". `read-file-desc` looks like "file
...
Fix `tt`: the first time the game runs, the limits of the arena are
not displayed.
...
Compare `emit-udga`, defined in the kernel, with the draft `.udga`,
...
Make `circle-pixel` throw an error by default. It cannot be a
...
b)` and `gxy>attr ( x y -- b)`, but the name notation used cannot be
...
=== 2015-06-07, 2018-06-16

Change: move `key` to the library, as `mode-key` or similar, and use a simpler
`key` in the kernel (e.g. `akey` from Afera).
...
=== 2016-04-24, 2017-05-06, 2017-12-09, 2018-01-03, 2018-03-01, 2018-06-05, 2018-06-10

Words that can be moved to the library:
`umin`, `umax`, `char+`, `char-`, `break-key?`,
`default-break-key?`, `get-drive`, `np!`...
...
Rewrite `m+` in Z80 (the version adapted from Z88 CamelForth doesn't
...
=== 2017-02-20, 2017-03-14, 2017-12-12, 2021-01-08
...
`farc!` is not used used in the kernel.

=== 2020-05-18

NOTE: Milestone: 0.15.0

Move `bank` to the library?

==== 2018-03-21

Improve needing of G+DOS directory descriptions to save blocks.
...
- Move the disk code, common only to +3DOS, G+DOS and TR-DOS, from
  <lib/dos.COMMON.fs> to <lib/dos.disk.COMMON.fs>. Keep in
  <lib/dos.COMMON.fs> only the code common also to NextZXOS.
- Move `read-block` and `write-block` to <lib/dos.disk.COMMON.fs>.
- Move `flush` to <lib/dos.disk.COMMON.fs>, because it must be
  included in the NextZXOS kernel.  
...
=== 2017-02-12, 2018-06-16

NOTE: Milestone: 0.15.0

Idea for a faster version of `(located`: Load only the first sector of
...
=== 2016-11-22, 2016-12-31, 2017-02-16, 2017-03-07, 2018-06-03
...
=== 2016-05-18, 2017-02-22, 2018-06-03

Support multiline block headers with an improved version of `(located)`, in
...
- Simple locals, by Wil Baden, 1987-01-01,
  <http://atariwiki.strotmann.de/wiki/Wiki.jsp?page=Local%20Variables>

=== 2018-04-14

Finish implementation of `(local)`, `locals|` and `{:`.
...
- Rename `cls-chars0` and `cls-chars1`.
- Rename `(load-program` to `load-program>`?
...
Homogenize the usage of prefix "far". In some words it's separated by
a dash.
...
- Move <fonts> to the root.
- Make <img> and move <screenshots> and <backgrounds> to it.
...
Finish `turnkey` and friends, or remove them.
...
Fix `see`: it recognizes an initial code word, e.g. `see over`, but
...
but it is used by `lastblk`, which does not need initialization).
