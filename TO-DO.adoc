...
- Use a single space before semicolon.
...
----
: find-file  ( ca len -- x | 0 )  file-status 0= and  ;
----

.2017-02-12:

Rename `file-exists?` to `find-file` (after `find-name`).
...
Sometimes, the words that depend on `file-status` start returning ior
#-1022 ("Wrong drive"). The error conditions are not clear yet. It
seems it happens after changing the disk.
...
Fix: `cat` crashes if there's no disk in the drive.  The chrash is
caused because the ROM exits to BASIC with message "CHECK DISK in
drive", and the lower screen has no lines (the system variable 23659
is set to 0 in Forth. There must be a way to check the drive first and
throw an exception.
...
.2017-01-05, 2017-02-08:

Use `plusd-in,` instead of `patch hook,`.
...
Confirm if "file descriptor" is correct. Meanwhile, "File Description
Area" is used instead.

.2017-02-09:

Having implemented `drive`, to convert a 0-index drive to the actual
identifier needed by the current DOS, the notation used for TR-DOS in
those cases must be decided:

|===
| DOS                                       | First drive | Last drive

| G+DOS                                     | 1           | 2
| TR-DOS (in filenames and BASIC commands)  | 'A'         | 'D'
| TR-DOS (in Z80 code)                      | 0           | 3
| +3DOS                                     | 'A'         | 'P'
|===

Using 0..3 is simpler, because the assembly interface needs no change.
The question is if notation 'A'..'D' must be explicit in filenames.
