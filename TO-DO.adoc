...
Factor `create c,`, which is used 11 times. This will save 13 bytes:
11 cells minus 9 bytes of the new definition.
...
: m3p ( 8b "name" -- )
  create c,
  does> ( reg -- ) ( reg pfa ) %1111110 and c@ swap 8* + c, ;
...
allowed, like `c pop,`). But `m3p` needs more memory than `af`.
...
Rename `sector-id` to `block-drive-sector-id`, because in TR-DOS and
+3DOS its a generic reusable calculation: it depends on the space that
can be used for blocks.

Write a generic `sector-id` as a factor of `block-drive-sector-id`. In
G+DOS both words can be aliases.

This change is related to the future implementation of block files on
TR-DOS and +3DOS.
