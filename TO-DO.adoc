...
- Convert the commands into cross-references, e.g. make `need
  make-thru-index` 2 cross-references to its words.
- Mention `load-program` in "How to write Forth programs".
...
=== 2018-03-04, 2020-07-07

NOTE: Milestone: 0.14.0

Use Asciidoctor `ifdef::[]` to customize the documentation of
`far-banks` (e.g. add a link to `default-1346`), `farlimit`, etc.,
depending on the current DOS.
...
____
WARNING: ``XXX`` is a state-smart word (see `state`).
____
...
See <https://forth-standard.org/standard/core/SMDivREM>.
