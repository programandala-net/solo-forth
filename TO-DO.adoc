...
Remove the non-standard default implementation of `to`, `value`, etc,
which is syntactic sugar for `!>`, `constant`, etc. Convert it into an
explicit layer, activated by needing a fake word, and move it to
module of the standard `value`.

Add non-standard `cvalue`.
