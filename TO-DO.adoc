...
Check: `nest-source` gets `source`, which is the result of a
calculation, but `unnest-source` restores it into `input-buffer`.
...
- Pass `version` as a parameter to Asciidoctor calls in Makefile,
  instead of replacing a markup in the source..
...
In this case, the first error is not catched:

----
: t2 s" 333 xxx yyy" evaluate ;
' t2 catch
yyy ?
#-256
----
...
----
s" 333 XXX yyy" evaluate
' t2 catch
xxx ?
#-256
----

DX-Forth has the same bug.

pForth "explodes":

----
: t s" 333 XXX 666" evaluate ;    ok
Stack<10>
' t catch XXX  ? - unrecognized word!
...
The current definition of `evaluate` is the following:
...
The new simpler version makes no difference:
...
Move to the kernel: `0exit`.

Move to the library: `?` (in Z80 or Forth).

Combine <memory.bank.fs> and <memory.far.fs>.

=== 2020-05-18

NOTE: Milestone: 0.14.0

Move to the library: `bank`.
...
Don't use `pad`. Leave it for the user or the editors. The following
words still use `pad`:
...
but its is used by `lastblk`, which does not need initialization).
