...
Move `upper`, `uppers` and `faruppers` to the library.

Move `c!bank`, `@bank` and `c@bank` to the library.
...
Decide a naming convention for constants that return addresses of
kernel routines. `default-bank-routine` seems clear, but others use no
hyphen and no "-routine" suffix.
