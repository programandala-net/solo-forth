...
Try to adapt the labels of the 8086 hForth assembler. Already started in the
drafts directory.
...
works fine. This problem happens with the high-level version of `paper>attr`
and `attr>paper`, which need `lshift` and `rshift`, which need `?(`, which
needs `str=`... But no exception is issued when `need order` is given before
`need paper>attr`...
