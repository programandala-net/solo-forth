...
- Make "true" and "false" cross-references to `true` and `false`.
...
- Rename `latest-wordlist` to `last-wordlist`, because of the analogy
  with `last` and `wordlist>last`, which return the address of the
  value.
...
Fix: load a version of `allocate` to test it with the Forth-2012 Test
Suite.
...
Fix: `m+` doesn't pass the Forth-2012 Test Suite.
