...
Faster, Z80 versions of words that fetch from and store to memory banks.

Move the following words to the library: `there`, `;code`, `cliteral`,
`2over`.

Study how to move `line>string` and `undefined?` to the library.  They are not
used in the kernel, but the are needed by the `need` utility.
...
Write `behead  ( xt -- )` or `behead  ( "name" -- )`. See DX-Forth and
others. `hided  ( nt -- )` is already in the kernel.
