...
It seems every action, e.g. `write-file`, must open the file, set its
pointer, do the operation and close the file again... An indexed or
linked list of UFIAs will be needed.

Some ideas:

Temporary method: Make `ufia` a constant that points to the UFIA used
by the current file...

Final method: Make `ufia` a data structure, in order to reuse its
fields with additional UFIAs, which will be used as file identifiers.
...
==== 2017-12-05, 2018-03-03

Fix: `cat` crashes the system. The problem seems to be the return from
`((acat`, but everything looks right. All this worked some time ago
