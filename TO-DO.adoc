...
----
: drive  ( x -- x' )  1+  ;             \ G+DOS
' noop alias drive  ( -- )  immediate   \ TR+DOS option 1
: drive  ( x -- x' )  'A' +  ;          \ TR-DOS option 2
: drive  ( x -- x' )  'A' +  ;          \ G+DOS
----
...
Fix: `cat` crashes if there's no disk in the drive.
...
.2016-12-30, 2017-01-23, 2017-02-09:

NOTE: Milestone 0.14.0

Try and finish the implementation of `set-block-drives`.
Then build the secondary disk images without the library.
Also, think how to update `last-locatable`.
