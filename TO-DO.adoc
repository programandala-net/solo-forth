...
This code in a screen leaves two zeros:
...
New verions of `scan`, `skip`, `parse`, `parse-name` and `word` have
been written, selectable with conditional compilation. Permanent
copies of those words have the "x" suffix added, for debugging.
...
New: `find-name` ( ca len -- ca len 0 | cfa 1 | cfa -1 )

.2015-09-18:

Fix: when loading blocks with the new parsing method, the parsing
stops at the end of the first block of the screen, because the null
word is not executed anymore. The solution is to use `refill`.
