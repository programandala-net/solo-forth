...
`;code` is standard and can not be reused, but anymay a simpler syntax
...
`>name` searches all word lists backwards. Therefore the first match
is the most recent alias or synonym:

----
synonym clear page
' page >name .name \ displays "clear"
----

Solution 1: Examine all words of the system, and return the last
match, not the first one.  Pro: Simple. Con: Always slow, no matter
the size of the dictionary.

Solution 2: Mark aliases and synonyms with a bit in the header.  Pro:
Definitive and useful for other needs. Con: One byte must be added to
the header.

Solution 3: Distinguish aliases and synonyms using the content of its
code field address, e.g. using a specific jump, or a jump with a
previous Z80 noop. Cons:

- every alias or synonym uses 3 or 4 bytes of data space.
- small time delay for the jump, as deferred words.
- distinguishing an alias or synonym from their "original" or from a
  deferred word needs more calculations.

Anyway, the problem can not be fully solved, because there could be
several aliases and synonyms of one single word.
...
Reduce the number of aliases in the libray.
