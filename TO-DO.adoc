...
----
  ...
  : cexchange  ( ca1 ca2 -- )  2dup c@ swap c@  rot c! swap c!  ;
    \ Exchange the 8-bit contents of ca1 and ca2.
----
...
Rename <environmental_queries.fsb>.

Add missing "misc" and "common" parts of module filenames.

Rename "misc" and "common" part of module filenames to uppercase.
...
Improve `internal external module` with new names and several versions of
`module`:

1. Ignore the internal words, bypassing them (current behaviour).
2. Remove the internal words, modifyng the headers pointer.
3. Behead the internal words (this is no advantage).

- `private end-private` to mark the private/internal words
- `forget-private` to forget the words, changing `hp`
- `unlink-private` to unlink the words
- `hide-private` to hide the words
- `behead-private` to behead the words

In fact <modules.privatize.fsb> could be combined with <modules.module.fsb>,
because `privatize` is the equivalent of the planned `behead-private`.
