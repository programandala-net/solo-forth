...
Make the manual contain only the contents related to the corresponding
DOS.
...
Find a solution to the duplicated entries of the glossary. All af them
...
Remove the file that contains only the glossary heading.  Instead of
using `cat` in <Makefile>, convert the main file of the manual into an
skeleton that includes the individual files.

.2017-02-18:

Fix rendering of `-->` in the documentation.

.2017-02-18:

Asciidoctor bug?: role _external_ in <README.adoc> is not rendered
into an HTML class when this file is included into the manual. It is
included verbatim.
...
Make `use-default-need`, `use-default-located` and `use-no-index`
optional, and the first time set the default actions without them.
This saves 102 bytes!
