...
Fix?: creating a duplicate name in the same vocabulary gives no error
if the vocabulary is not in the search order.

Change: make `word` return a string.
...
Fix: `cold` does not restores the dictionary.

Change: remove the control structure checking (`?pairs`).

Fix: crash at the start! Why? What did I change?
