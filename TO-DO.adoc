...
- Benchmark `cell+ cell+` (from `switcher`) and `[ 2 cells ] literal +`.
...
The machine resets when the second string of `search` is empty.
...
Remove `don't` or move it to <flow.misc.fsb>

Move `times` and `dtimes` to <flow.times.fsb>
