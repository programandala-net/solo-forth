...
- Table cells justification (both notations: `cols` and `>|`) and
  columns percentage width are ignored by Asciidoctor EPUB3. They are
  correct in the PDF made by Asciidoctor PDF. For example, see
  `specforth-editor` and `banks`.
- Compress the PDF manuals.

=== 2020-06-12, 2020-07-07

NOTE: Milestone: 0.14.0

- Convert the commands into cross-references, e.g. make `need
  make-thru-index` 2 cross-references to its words? Problem: the style
  would be affected.
- Fix column widths in "Commands to rebuild the manual" and other
  tables.
...
Resize the columns of the editor commands tables to make the stack
notes of their commands fit.
...
=== 2017-12-12, 2020-06-16

NOTE: Milestone: 0.14.0

Add cross-references to `defer` to all deferred words. Already done in
the kernel. Better yet: add "(see `defer`)" after "is a deferred
word".
...
=== 2017-02-24, 2020-06-16

NOTE: Milestone: 0.14.0

Update "Note:" to Asciidoctor markup "NOTE:" where appropiate.
