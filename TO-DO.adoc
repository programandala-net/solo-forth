...
Adopt the following notation for Z80 flags:

[horizontal]
Fs :: Sign flag (bit 7)
Fz :: Zero flag (bit 6)
Fh :: Half Carry flag (bit 4)
Fp :: Parity/Overflow flag (bit 2)
Fn :: Add/Subtract flag (bit 1)
Fc :: Carry flag (bit 0)
...
[caption="Main words implemented on each DOS"]
|===
| Word              | G+DOS | TR-DOS | +3DOS
...
The kernel entry `dos.c` prepares the A register and Fz flag after the
value in the C register, but not all command code routines return an
trror in C. Confirm this and study if it would be simpler to leave the
error management to the calling word.

.2017-03-09, 2017-03-10:

New bug: The origin can not be moved up. Some time ago it was
possible. Details are in the kernel.

It doesn't happen every time.

.2017-03-08:

Fix `(acat`: the last disk error, e.g. when `delete-file` does not
find a file, is remembered by the DOS, and `(acat` returns it in the A
register.
...
Implement the TR-DOS command `LIST` as `cat`.
...
.2017-02-08, 2017-03-06:

Fix `tape-file>`: when the file length attribute is not zero
