...
New: `cvariable`, `cconstant`.

Fix: `attr`.

Improve: version in the greeting message.

Fix: warm reentry crashes the system.

.2015-08-14:

Fix: problems when true=-1.
...
Everything seemed to work fine: the 1-buffer system, true=-1...  But
trying to fix the bug of warm something brokes, and the system crashes
at start.  It seems the error conditions are not always the same. The
debugger didn't help.


.2015-08-18:

Still problems after migrating to GNU binutils.

Does the `np` symbol is updated in memory? Why it does not appear in
the symbols list?  It seems it depends on several conditions related
to code sections.
