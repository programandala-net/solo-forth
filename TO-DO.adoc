...
Rewrite `attr` and `attr-addr` with opcodes, without assembler.
Search for similar cases, e.g. `lshift`.
...
Update 2016-12-30: Already solved? It seems the problem was the `grep`
expression in the `fs2fsb.sh` converter, because of the file enconding.
...
The games does not fit a TR-DOS disk, because the library has grown.

Extract them, make them independent projects?

First, convert them to .fs as planned, and load them with `load-app`.
...
`lib-order`, `set-lib-orde`, `get-lib-order` are in the kernel, but no used
yet.
...
Update 2016-12-31: `need\` is clearer than `parses` to parse the current line.
...
Write `wordlists` to list all word list defined.
