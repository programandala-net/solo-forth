...
Add `1/string` to the kernel, in Z80.  The idiom `1 /string` is used 5 times in
the kernel (by `number-base`, `>number` and `number?`). Replaces them with
`1/string` will save 10 data-space bytes (`1` is a code word) and the code
would run faster. This word needs only 5 bytes of data space, because its
execution runs into `/string`:
...
The idiom `1 /string` is used also in the library: once in `substitute` and
twice in `get-substitution`.
