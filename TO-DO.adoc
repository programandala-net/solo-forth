...
- Convert the definition lists of exception codes into tables.
- Remove the OS exception codes? They are supported only because some of them
  may be returned by G+DOS commands. At least, explain this.

=== 2018-06-09

NOTE: Milestone: 0.14.0

- Remove section _Summary of the DOS support_. Use the table of section
  _Comparative of DOS support_ instead.
- Confirm corrupted links with backslash and remove the corresponding note.

=== 2018-06-05, 2018-06-09

NOTE: Milestone: 0.14.0

- Make a different version of the manual for every DOS, not only the glossary.
  Use Asciidoctor's conditional preprocessor directives and the command-line
  option `--attribute`.
- Split sections _Disks_, _How to run_, _About the disk image formats_...

=== 2018-04-14

NOTE: Milestone: 0.14.0

Improve the width of table columns.
...
=== 2018-02-14, 2018-02-28, 2018-03-11, 2018-06-03

One 640-KiB TR-DOS disk is barely enough to contain the library.

To compact the code:

- Implement the multiline block headers.  This will make more long-name short
  definitions fit into one block.
- Move the stack comments to a meta comment below the code. This will make many
  one-liners fit one line.
- Use `needing X 0(` instead of `unneeding X ?(`, i.e. with zero-version of the
  conditional comments.
- Implement `need(` to compact the requirements
