...
Improve `substitute`: hide factors; rename words; make it faster; don't use a
buffer for the name, but the substring.
...
The standard version of `replaces` creates a 255-byte buffer for every
substitution.  Most of the times that is not needed.  Write two alternatives,
variants or configurable behaviours of `replaces`:

1. Accept an xt as substitution. The execution of the xt will return the actual
   string.
2. Create a buffer as long as the substitution. Therefore the substitution can
   not be replaced by longer substitutions later.
...
Decide a convention about permanent and temporary attributes. At the moment the
attribute words defined in the kernel (`bright`, `flash`, `paper` and `ink`)
change both attributes, but `overprint` and `inverse`, defined in the library,
change only the temporary attributes.

.2017-01-22:

Write `temp-attr@  ( -- b )`, `temp-attr@  ( b -- )`, `perm-attr@  ( -- b )`
and `perm-attr@  ( b -- )`.
