...
Make `mode32-emit` configurable with a patch:

- Mode 1 (current): Print characters 32..127 from the ordinary font,
  and characters 128..255 from the UDG set.
- Mode 2 (new): Print characters 32..255 from the ordinary font.
...
Possible solution: Make UDG 128..255 be the same in both index modes,
i.e., UDG 0..127 can be accessed only with "0udg" words, while UDG
128..255 don't vary. This is better than the current behaviour, when
UDG 128 is UDG 0 in a 0-based UDG word. In fact, this solution makes
128-based UDG words unnecessary.
...
If `base` were not a user variable, `binary` and `decimal` would be
smaller in Z80 than in Forth.
...
Convert `ocr-chars` and `ocr-first` to character variables.
