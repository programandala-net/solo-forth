...
New:
ANS Forth `leave` --> `exit-loop` / `break-loop` (code from DZX-Forth).

Fix: `65 dup emit .`. `.` prints a space at the start.
In Abersoft Forth it does not.
The code of `d.r` is the same in Solo Forth and Abersoft Forth.
