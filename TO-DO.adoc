...
Fix compatibility problem between both assemblers: compilation of `rdraw176`
fails when `z80-asm` is already loaded.

Example: `rdraw176` needs `z80-asm,`:

This works fine:
`1 load need z80-asm, need z80-asm need rdraw176`.

But this fails:
`1 load need z80-asm need z80-asm, need rdraw176`
because `rif` does not do its job.
...
When `>name` is used with the xt of an alias, the returned name
is that of the oldest 
of aliases are taken
