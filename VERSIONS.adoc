...
- Rename `pause` to `?frames`, and `do-pause` to `frames`.  The name
  "pause" was taken from BASIC but the new names are more clear, and
  consistent with `ms` and `seconds`. Add a new `pause` after Sinclair
  BASIC.  Just a convenience to adapt BASIC programs.
- Improve `see`: type strings between quotes.  This is more clear when
...
- Rename the far-memory routines constants.  A more clear convention
