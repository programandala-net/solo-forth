...
Solo Forth is a standard Forth system for the ZX Spectrum 128 and
compatible computers, with disk drives and +3DOS, G+DOS, or TR-DOS.

Solo Forth can not run on the original ZX Spectrum 48, but could be
...
1. *I wanted to program the ZX Spectrum with a modern Forth system*:
   ...
   Forths, and discover some bugs of the ZX Spectrum port. Then I
   ...
   comfortable for cross development.  In fact both projects were
   parallel for months. The objective was reached but, after a
...
The <disks> directory of the <<_project_directories,directory tree>> contains the
disk images:
...
1. Run a ZX Spectrum emulator and select a Pentagon
   128{blank}footnote:diskformat[Make sure the disk drives are configured as
...
1. Run a ZX Spectrum emulator and select a Pentagon
   1024{blank}footnote:diskformat[].
2. “Insert” the disk image file
...
3. Choose "128k menu"footnote:pentagonboot[] from the computer
   ...
   command line{blank}footnote:trdoscli[].
4. Press the '.' key to get the ``RUN`` command and press the Enter
   key.  Solo Forth will be loaded from disk.
...
1. Run a ZX Spectrum emulator and select a ZX Spectrum 128 (or ZX
   Spectrum +2) with the Beta 128 interface{blank}footnote:diskformat[].
2. “Insert” the disk image file <disks/trdos/disk_0_boot.trd> as disk
