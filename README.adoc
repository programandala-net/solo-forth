...
1. *I wanted to program the ZX Spectrum with a modern Forth system*:
   ...
   parallel for months. Finally the objective was reached but, after a
...
2. *Nobody had written such a Forth system before:* In 2015 there was
   ...
   Forth written for ZX Spectrum 128 was Lennart Benschop's Forth-83
   (1988). But despite being more powerful than fig-Forth, it was
   still tape-based (or microdrive-based) and used a RAM disk to keep
   the sources, basically the same method used by all ports of
...
(a port from CP/M DX-Forth to ZX Spectrum +3e).
...
from the very beginning.  Also, there's a
