...
Solo Forth is a Forth system for a wide range of ZX Spectrum
compatible computers and four DOSes:

- ZX Spectrum 128/+2 with G+DOS or TR-DOS.
- ZX Spectrum +2A/+2B/+3/+3e with +3DOS.
- Pentagon 128/512/1024 with TR-DOS.  
- Scorpion ZS 256 with TR-DOS.  
- ZX Spectrum Next with NextZXOS.

Solo Forth cannot run on the first ZX Spectrum model (16 or 48 KiB),
but in theory it could be used to develop programs for it. 
...
// XXX REMARK -- This tag does not include the title, because this is
// the only entry for G+DOS, and the computers are mentioned in item
// 1:
...
// On ZX Spectrum Next {{{3
==== On ZX Spectrum Next

// tag::run_zx_spectrum_next[]

// XXX REMARK -- This tag does not include the title, because this is
// the only entry for NextZXOS, and the computer is mentioned in item
// 1:
...
// tag::library_on_nextzxos[]

1. <<_run_nextzxos,Run Solo Forth>>.
2. Type `s" solo.fb" open-blocks` to open the blocks file.
3. Type `1 load` to load block 1 from the library. By convention,
   block 0 cannot be loaded (it is used for comments), and block 1 is
   used as a loader.  In Solo Forth, block 1 contains `2 load`, in
   order to load the `need` tool from block 2.
4. Type `need name`, were “name” is the name of the word or tool you
