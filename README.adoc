...
2. Insert the library disk: ** On G+DOS: "Insert" the file
   ...
   one.  ** On TR-DOS: "Insert" the file
   <disks/trdos/disk_1_library.trd> as disk B of the Beta 128
   interface. Type `1 set-drive throw` to make drive 1footnote:[The
   TR-DOS BASIC interface uses letters 'A'..'D' to identify the disk
   drives, in commands and filenames. But, under the hood, TR-DOS uses
   numbers 0..3 to identify the disk drives, and filenames don't
   include the drive letter. This is the way Solo Forth works too.
   Usage of '.'..'.' instead of 0..3 maybe implemented in a future
   version of Solo Forth, either by default or as an option.] the
   current one.  ** On +3DOS: "Insert" the file
   <disks/plus3dos/disk_1_library.dsk> as disk A.
...
files, is a work in progress. At the moment it contains only a short
...
- Make all Forth words links to the glossary entries.  Currently only
  Forth words mentioned in the glossary are cross references.
