...
need >amark need unresolved
  \ XXX TMP -- For absolute references, until `l:` is improved.
...
code zx7s ( a1 a2 -- )
  ...
  \ dzx7s_main_loop:
  2 l: 0 call, >amark 0 unresolved ! c? runtil
  \         call dzx7s_next_bit
  \         jr   nc, dzx7s_copy_byte_loop ; next bit indicates either literal or sequence

  \ ; determine number of bits used for length (Elias gamma coding)
  d push, 0 b ldp#, b d ld,
  rbegin d inc, 0 call, >amark 1 unresolved ! c? runtil
  \         push de
  \         ld   bc, 0
  \         ld   d, b
  \ dzx7s_len_size_loop:
  ...
  rbegin 0 nc? ?call, >amark 2 unresolved !
    c rl, b rl, 1 l# c? ?jr, d dec,
         z? runtil b incp,
  ...
  \ dzx7s_rld_next_bit:
  rbegin 0 call, >amark 3 unresolved !
  d rl, c? runtil d inc, d srl,
  \         call dzx7s_next_bit
...
( zx7s )
  ...
  0 call, >amark 4 unresolved ! exx, jpnext,
  \         call dzx7s_next_bit
  \         exx                           ; restore Forth IP
  \         _jp_next                      ; exit to next word

  \ dzx7s_next_bit:
  here 0 unresolved @ !  here 1 unresolved @ !
  here 2 unresolved @ !  here 3 unresolved @ !
  here 4 unresolved @ !

  a add, nz? ?ret, m a ld, h incp, rla, ret, end-code
...
need assembler need l:
    ...
    \ XXX FIXME -- `0 l#` is not properly resolved, because
    \ labels works only with relative references.
