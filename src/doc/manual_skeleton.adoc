...
NOTE: By default, only the exception code is displayed by an uncatched `throw`,
because the default action of `.throw` is `.throw#`. In order to display also
the error description, `.throw-message` must be loaded from the
<<_library,library>>.
