...
No special requirement is necessary. Any ZX Spectrum Next can run Solo
Forth.
...
| ZX Spectrum 128/+2/+2A/+2B/+3/+3e | divIDE/divMMC/ZXATSP/ZXCF/ZXMMC...  | ResiDOS
| ZX Spectrum 128/+2/+2A/+2B/+3/+3e | divIDE/divMMC/ZXATSP/ZXCF/ZXMMC...  | esxDOS
| ZX-UNO/UNO+/DOS                   |                                     | esxDOS
|===
...
+3DOS uses 16 KiB of banked memory, but it has good features:
...
beginning of the project), and it's also the author's platform of
choice to develop projects in Solo Forth, because of its excellent
features:

- Big disk capacity (800 KiB for Forth blocks or 780 KiB for files).
- It uses only the RAM of the Plus D interface.
- Simple disk image format, without metadata.  
- Fast disk operations.
...
Support for NextZXOS was not planned at all. Solo Forth was designed
as a diskette-based Forth system for ZX Spectrum 128 and compatible
computers, storing disk blocks directly on disk sectors.

In fact, during some months the plan was to start a new Forth designed
for NextZXOS from scratch, with a different approach.

But being NextZXOS a compatible evolution of +3DOS, the +3e ROM and
IDEDOS, it was clear that it wouldn't be difficult to make a NextZXOS
version from the current +3DOS one.

Running in NextZXOS on the ZX Spectrum Next, Solo Forth is more
powerful and easier to use than in +3DOS, G+DOS or TR-DOS.

endif::[]

ifdef::trdos[]

TR-DOS is more than 5 times slower than +3DOS, G+DOS or NextZXOS, it
uses 368 bytes of RAM from the main computer memory, and the maximum
...
// XXX TODO Complete the data about NextZXOS:

[%autowidth]
.Main features of every DOS
|===
| Feature                       | +3DOS           | G+DOS   | NextZXOS      | TR-DOS

| Maximum number of disk drives | 16 (actually 2) | 2       | 16            | 4
| Maximum raw disk capacity     | 780 KiB         | 800 KiB | ?             | 640 KiB
| RAM used                      | 16 KiB (bank)   | 0       | 16 KiB (bank) | 368 B
| Block disk operations time    | 1.0             | 1.2     | ?             | 5.1
| Z80 assembly API              | Excellent       | Poor    | Excellent     | Poor
| API documentation             | Excellent       | Good    | Excellent     | Poor
|===
...
[cols="18,10,10,10,10,52"]
.Main disk-related words implemented
|===
| Word                   | +3DOS | G+DOS | NextZXOS | TR-DOS | Description

|
ifdef::plus3dos[`2-block-drives`]
ifdef::gplusdos[`2-block-drives`]
ifdef::nextzxos[``2-block-drives``]
ifdef::trdos[`2-block-drives`]
| YES | YES | n/a | YES | Use the first two drives as block drives

|
ifdef::plus3dos[``3-block-drives``]
ifdef::gplusdos[``3-block-drives``]
ifdef::nextzxos[``3-block-drives``]
ifdef::trdos[`3-block-drives`]
| n/a | n/a | n/a | YES | Use the first three drives as block drives

|
ifdef::plus3dos[``4-block-drives``]
ifdef::gplusdos[``4-block-drives``]
ifdef::nextzxos[``4-block-drives``]
ifdef::trdos[`4-block-drives`]
| n/a | n/a | n/a | YES | Use the first four drives as block drives
...
| `acat`
| YES | YES | YES | YES | Display an abbreviated disk catalogue

|
ifdef::plus3dos[`bank-read-file`]
ifdef::gplusdos[``bank-read-file``]
ifdef::nextzxos[`bank-read-file`]
ifdef::trdos[``bank-read-file``]
| YES | | YES | | Read from a file with a bank paged in

|
ifdef::plus3dos[`bank-write-file`]
ifdef::gplusdos[``bank-write-file``]
ifdef::nextzxos[`bank-write-file`]
ifdef::trdos[``bank-write-file``]
| YES | | YES | | Write to a file with a bank paged in
...
| `cat`
| YES | YES | YES | YES | Display a detailed disk catalogue

|
ifdef::plus3dos[`close-file`]
ifdef::gplusdos[``close-file``]
ifdef::nextzxos[`close-file`]
...
ifdef::nextzxos[`create-file`]
ifdef::trdos[``create-file``]
| YES | | YES | | Standard Forth

| `delete-file`
| YES | YES | YES | YES | Standard Forth

|
ifdef::plus3dos[`drive-unused`]
ifdef::gplusdos[`drive-unused`]
ifdef::nextzxos[`drive-unused`]
ifdef::trdos[``drive-unused``]
| YES | YES | YES | | Return the number of unused kibibytes in a drive

|
ifdef::plus3dos[``file-dir#``]
ifdef::gplusdos[``file-dir#``]
ifdef::nextzxos[``file-dir#``]
...
ifdef::nextzxos[`flush-drive`]
ifdef::trdos[``flush-drive``]
| YES | | YES | | Write all pending data for a drive

| ``flush-file``
| | | | | Standard Forth

| `get-block-drives`
| YES | n/a | YES | YES | Get the drives used as block drives

| `get-drive`
| YES | YES | YES | YES | Get the number of the current drive
...
ifdef::nextzxos[``read-byte``]
ifdef::trdos[``read-byte``]
| YES | | YES | | Read byte from file

|
ifdef::plus3dos[`read-file`]
ifdef::gplusdos[``read-file``]
ifdef::nextzxos[``read-file``]
...
ifdef::nextzxos[``reposition-file``]
...
ifdef::plus3dos[`set-block-drives`]
ifdef::gplusdos[`set-block-drives`]
ifdef::nextzxos[``set-block-drives``]
ifdef::trdos[`set-block-drives`]
| YES | n/a | YES | YES | Set the drives used as block drives

| `set-drive`
| YES | YES | YES | YES | Set the current drive

|
ifdef::plus3dos[``undelete-file``]
ifdef::gplusdos[``undelete-file``]
ifdef::nextzxos[``undelete-file``]
...
ifdef::nextzxos[`wacat`]
ifdef::trdos[``wacat``]
| YES | YES | YES | | Display an abbreviated disk catalogue with wildcards
...
ifdef::nextzxos[`wcat`]
ifdef::trdos[``wcat``]
| YES | YES | YES | | Display a detailed disk catalogue with wildcards

|
ifdef::plus3dos[`write-byte`]
ifdef::gplusdos[``write-byte``]
ifdef::nextzxos[`write-byte`]
ifdef::trdos[``write-byte``]
| YES | | YES | | Write a byte to a file

|
ifdef::plus3dos[`write-file`]
ifdef::gplusdos[``write-file``]
ifdef::nextzxos[`write-file`]
...
- http://programandala.net/en.program.solo_forth.html[Fossil
  repository,role="external"]
...
The <nextzxos> directory of the <<_tree,directory tree>>
...
- <solo.bas> is the BASIC loader, the program that launches Solo
  Forth.
- <solo.bin> is the Solo Forth kernel loaded by <solo.bas>.
- <solo.fb> contains the library, some programs (little sample games,
  most of them under development, two block editors and one sound
  editor), tests and benchmarks (most of them were used during the
  development and their only documentation is the commented source).

// XXX TODO Improve, mention the block editors and maybe Tasword:

WARNING: <solo.fb> is a Forth blocks file: It contains the source
Forth blocks without any control character. Therefore its content
cannot be accessed with ordinary editors.
...
ifdef::nextzxos[]
include::doc.README.linked.adoc[tags=library_on_nextzxos]
endif::nextzxos[]
...
will automatically start loading the first block where the “name”
is defined

// XXX TODO: Add NextZXOS:
...
// XXX TODO -- Combine with section "How to load a program that uses the library"
...
The <solo.fb> file contains also many little specific tests and
benchmarks used during the development of Solo Forth, probably not
interesting for the application programmer.  But it also contains an
adapted version of the Hayes test and some known benchmarks.

endif::[]

// First, set the required block disks / open the blocks file {{{2
ifdef::plus3dos,gplusdos,trdos[]
=== First, set the required block disks
endif::[]
ifdef::nextzxos[]
=== First, open the blocks file
endif::[]
...
. <<_run,Run Solo Forth>> or enter `cold` to start from scratch.
. Open the blocks file with `s" solo.fb" open-blocks`.
. Enter command ``1 load`` to `load` the `need` tool.
...
| NextZXOS | ZX Spectrum Next  | ``make nextzxos``
...
// XXX FIXME: The table is too wide. Show only the current DOS or
// split the content into several tables or lists:

[%autowidth]
.Commands to rebuild the manual
|===
| Format  | +3DOS                 | G+DOS                 | NextZXOS              | TR-DOS             | All

| DocBook | ``make plus3dosdbk``  | ``make gplusdosdbk``  | ``make nextzxosdbk``  | ``make trdosdbk``  | ``make dbk``
| EPUB    | ``make plus3dosepub`` | ``make gplusdosepub`` | ``make nextzxosepub`` | ``make trdosepub`` | ``make epub``
| HTML    | ``make plus3doshtml`` | ``make gplusdoshtml`` | ``make nextzxoshtml`` | ``make trdoshtml`` | ``make html``
| ODT     | ``make plus3dosodt``  | ``make gplusdosodt``  | ``make nextzxosodt``  | ``make trdosodt``  | ``make odt``
| PDF     | ``make plus3dospdf``  | ``make gplusdospdf``  | ``make nextzxospdf``  | ``make trdospdf``  | ``make pdf``
| All     | ``make plus3dosdoc``  | ``make gplusdosdoc``  | ``make nextzxosdoc``  | ``make trdosdoc``  | ``make doc``
|===
...
| -999..-256    | Solo Forth
| -1127..-1000  | {dosname}
| -1154..-1128  | Sinclair BASIC
| -4095..-1155  | Solo Forth
| -32768..-4096 | Programs
|===

// XXX TODO Confirm that Sinclair BASIC errors are properly included,
// and indexed by `error>ordinal`.

The original Sinclar BASIC error codes are included (in range -1154..-1128)
...
include::doc.nextzxos.exception_codes.adoc[]
