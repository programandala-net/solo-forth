...
- One double-sided 80-track disk drive (two recommended), for 720-KiB
  disks.  Therefore, the original single-sided 40-track disk drives
  (173 KiB per disk side) provided by the ZX Spectrum +3 are not
  supported.
...
- 128 KiB RAM.
- One double-sided 80-track disk drive (three recommended), for
  640-KiB disks. Other disk formats can be used, but only for files,
  not for block disks.
...
This is the {dosname} variant of the Solo Forth manual. Nevertheles, some
details about the rest of supported DOSes are included as well, when the
comparison is useful.

This manual is built automatically from the sources and other files. It is a
work in progress. At the moment it contains mainly a <<_description,description
of the Forth system>>, the basic <<_run,usage information>> and a complete
<<_glossary,glossary>> with cross references.
...
DOSfootnote:[Links to words that are not defined in {dosname} are inactive.]:
