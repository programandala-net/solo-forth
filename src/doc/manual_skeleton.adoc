...
the more advanced IDEDOS and ResiDOS in the future.
...
| `acat`            | YES   | YES    | n/a   | Abbreviated disk catalogue
| `bin`             |       |        | YES   | Standard Forth
| `cat`             | YES   | YES    | draft | Detailed disk catalogue
...
| `wacat`           | YES   |        |       | Abbreviated disk catalogue with wildcards
| `wcat`            | YES   |        | draft | Detailed disk catalogue with wildcards
...
following commands to load the Pong game:
...
. Enter command `need set-block-drives` to load `set-block-drives`.
. Depending on your DOS, configure the block drives as follows:
  ** On G+DOS: `2 1 2 set-block-drives`
