...
DOS{blank}footnote:[Links to words that are not defined in {dosname} are inactive.]:
...
| `file-dir#`       |       | YES   | YES    | Return the directory number of a file
...
| `flush-drive`     | YES   |       |        | Write pending data for a drive
